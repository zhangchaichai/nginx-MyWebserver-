...
{
    //因为listen套接字上用的不是ET【边缘触发】，而是LT【水平触发】，意味着客户端连入如果不处理，这个函数会被多次调用，所以，这里可以不必多次accept()，可以只执行一次accept()
    ...
    int                level;
    int                s;
    static int         use_accept4 = 1;   //认为能够使用accept4()函数
    ...
    {     
        ...
        if(s == -1)
        {
            ...
            {
                //除非你用一个循环不断的accept()取走所有的连接，不然一般不会有这个错误【我这里只取一个连接，也就是accept()一次】
                return ;
            } 
            level = NGX_LOG_ALERT;
        ...
        //如果某些恶意用户连上来发了1条数据就断，不断连接，会导致频繁调用ngx_get_connection()使用我短时间内产生大量连接，危及本服务器安全
        if(m_connectionList.size() > (m_worker_connections * 5))
        {
            //比如你允许同时最大2048个连接，但连接池却有了 2048*5这么大的容量，这肯定是表示短时间内 产生大量连接/断开，因为我的延迟回收机制，这里连接还在垃圾池里没有被回收
            if(m_freeconnectionList.size() < m_worker_connections)
            {
                //整个连接池这么大了，而空闲连接却这么少了，所以我认为是  短时间内 产生大量连接，发一个包后就断开，我不可能让这种情况持续发生，所以必须断开新入用户的连接
