...
{
    ++iCurrsequence;   
    if(precvMemPointer != NULL)//我曾经给这个连接分配过接收数据的内存，则要释放内存
    {        
...
lpngx_connection_t CSocekt::ngx_get_connection(int isock)
{
    ...
    //因为我要采用延迟释放的手段来释放连接，因此这种 instance就没啥用，这种手段用来处理立即释放才有用。
...
//有些连接，我不希望马上释放，要隔一段时间后再释放以确保服务器的稳定，所以，我把这种隔一段时间才释放的连接先放到一个队列中来
...
void* CSocekt::ServerRecyConnectionThread(void* threadData)
{
    ...
    lpngx_connection_t p_Conn;
   // ngx_log_stderr(0,"处理连接回收的线程  -------  进程和线程的id: %d %L\n",getpid(),syscall(SYS_gettid));
    while(1)
    {
        //为简化问题，我直接每次休息200毫秒
...
lblRRTD:
            ...
            for(; pos != posend; ++pos)
            {
                ...
                //到释放的时间了: 
                //......这将来可能还要做一些是否能释放的判断，先预留位置
                //....

                //我认为，凡是到释放时间的，iThrowsendCount都应该为0；这里我加点日志判断下
...
//用户连入，我accept4()时，得到的socket在处理中产生失败，则资源用这个函数释放【因为这里涉及到好几个要释放的资源，所以写成函数】
//我把ngx_close_accepted_connection()函数改名为让名字更通用，并从文件ngx_socket_accept.cxx迁移到本文件中，并改造其中代码，注意顺序
