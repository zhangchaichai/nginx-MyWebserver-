...
{   
    ...
    {
        p_Conn = (lpngx_connection_t)(m_events[i].data.ptr);           //ngx_epoll_add_event()给进去的，这里能取出
...
{    
    ...
    CMemory *p_memory = CMemory::GetInstance();
   // ngx_log_stderr(0,"处理发送消息队列的线程  -------  进程和线程的id: %d %L\n",getpid(),syscall(SYS_gettid));
    while(g_stopEvent == 0) //不退出
    {
        ...
        {
            err = pthread_mutex_lock(&pSocketObj->m_sendMessageQueueMutex); //要操作发送消息对列m_MsgSendQueue，所以这里要临界            
            ...
            while(pos != posend)
            {
                pMsgBuf = (*pos);                          //拿到的每个消息都是 消息头+包头+包体【但要注意，不发送消息头给客户端的】
