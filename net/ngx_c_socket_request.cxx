...
{  
    ...
    //ngx_log_stderr(0,"读包头的线程  -------  进程和线程的id: %d %L\n",getpid(),syscall(SYS_gettid));
    //收包，注意用的第二个和第三个参数，必须保证 c->precvbuf指向正确的收包位置，保证c->irecvlen指向正确的收包宽度
...
ssize_t CSocekt::sendproc(lpngx_connection_t c,char *buff,ssize_t size)  //ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    for ( ;; )
    {
        ...
        if(n > 0) //成功发送了一些数据
        {        
            //发送成功一些数据，但发送了多少，这里不关心，也不需要再次send
            //这里有两种情况
        ...
        if(n == 0)
        {
            ...
            //连接断开，并不在send动作里处理诸如关闭socket这种动作，集中到recv那里处理，否则send,recv都处理都处理连接断开关闭socket则会乱套
...
//设置数据发送时的写处理函数,当数据可写时epoll通知，在 int CSocekt::ngx_epoll_process_events(int timer)  中调用此函数
