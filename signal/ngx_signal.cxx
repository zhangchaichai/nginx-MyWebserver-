...
//数组 ，定义本系统处理的各种信号，取一小部分nginx中的信号，并没有全部搬移到这里，日后若有需要根据具体情况再增加
...
ngx_signal_t  signals[] = {
    ...
    { SIGSYS,    "SIGSYS, SIG_IGN",  NULL               },        //忽略这个信号，SIGSYS表示收到了一个无效系统调用，如果不忽略，进程会被操作系统杀死，--标识31
                                                                  //把handler设置为NULL，代表 我要求忽略这个信号，请求操作系统不要执行缺省的该信号处理动作（杀掉我）
    //...日后根据需要再继续增加
...
int ngx_init_signals()
{
    ...
    struct sigaction   sa;   //sigaction：系统定义的跟信号有关的一个结构，后续调用系统的sigaction()函数时要用到这个同名的结构
    ...
    {        
        ...
        if (sigaction(sig->signo, &sa, NULL) == -1) //参数1：要操作的信号
                                                     //参数2：主要就是那个信号处理函数以及执行信号处理函数时候要屏蔽的信号等等内容
                                                      //参数3：返回以往的对信号的处理方式【跟sigprocmask()函数边的第三个参数是的】，跟参数2同一个类型，这里不需要这个东西，所以直接设置为NULL；
        {   
...
static void ngx_process_get_status(void)
{
    ...
    for ( ;; ) 
    {
        ...
        pid = waitpid(-1, &status, WNOHANG); //第一个参数为-1，表示等待任何子进程，
                                              //第二个参数：保存子进程的状态信息(如果想详细了解，可以百度一下)。
